...
const Header = () => {
  ...
  const handleSubmenu = (index) => {
    setOpenIndex(openIndex === index ? -1 : index);  // Toggle submenu visibility
  };
        ...
        <div className="container">
          <div className="relative -mx-4 flex items-center justify-between">
            <div className="w-60 max-w-full px-4 xl:mr-12">
              ...
              >
                <span className={`w-full text-2xl font-bold ${sticky ? "text-dark dark:text-white" : "text-black dark:text-white"}`}>
                  VoteGuard.
                </span>
              </Link>
            </div>
                    ...
                    {dynamicMenuData.map((menuItem, index) => (
                      <li key={index} className="group relative">
                        ...
                        {menuItem.submenu && openIndex === index && (
                          <ul className="absolute left-0 mt-2 w-48 bg-white border border-gray-200 shadow-lg rounded-lg z-50 transition-all ease-in-out duration-300 opacity-0 transform translate-y-4 group-hover:opacity-100 group-hover:translate-y-0 dark:bg-dark dark:border-gray-700">
                            {menuItem.submenu.map((subItem, subIndex) => (
                              <li key={subIndex}>
                                <Link
                                  href={subItem.path}
                                  className="block px-4 py-2 text-sm text-dark hover:bg-gray-100 dark:text-white hover:dark:bg-gray-700"
                                >
                                  {subItem.title}
                                </Link>
                              </li>
                ...
                {!isLoggedIn ? (
                  ...
                  <div className="relative">
                    ...
                    <button
                      ...
                      className="w-10 h-10 rounded-full bg-white flex items-center justify-center text-black font-bold dark:bg-dark dark:text-white"
                    >
                      P
                    </button>
                    ...
                    {/* Dropdown Menu */}
                    {dropdownOpen && (
                      <div className="absolute right-0 mt-2 w-40 bg-white border border-gray-200 shadow-lg rounded-lg z-50 dark:bg-dark dark:border-gray-700">
                        <Link
                          href="/Profile"
                          className="block px-4 py-2 text-sm text-dark hover:bg-gray-100 dark:text-white hover:dark:bg-gray-700"
                        >
                        ...
                        <button
                          ...
                          className="w-full text-left block px-4 py-2 text-sm text-dark hover:bg-gray-100 dark:text-white hover:dark:bg-gray-700"
                        >
                          Logout
                        </button>
                      </div>
